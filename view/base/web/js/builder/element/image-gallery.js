	...
	var directive = function(cytraconBuilderUrl) {
		return {
			...
			templateUrl: function(elem) {
				return cytraconBuilderUrl.getTemplateUrl(elem, 'Cytracon_PageBuilder/js/templates/builder/element/image_gallery.html');
			},
			...
			link: function(scope, element) {
				...
				function initGallery() {
					...
					angular.forEach(_element.items, function(item, key) {
						...
						if (item.type == 'media') {
							newItem['url']  = cytraconBuilderUrl.getImageUrl(item.image);
							newItem['type'] = 'image';

							if (item.full_image) {
								newItem['full'] = cytraconBuilderUrl.getImageUrl(item.full_image);
							}
						}
						...
						if (item.type == 'video') {
							...
							if (item.image) {
								newItem['thumb'] = cytraconBuilderUrl.getImageUrl(item.image);
							}
						}
